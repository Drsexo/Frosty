...
set_perm "$MODPATH/doze.sh" 0 0 0755
...
if [ "$ENABLE_GMS_DOZE" -eq 1 ]; then
  ...
  # Create temp file for found XMLs
  TMPFILE="$MODPATH/found_xmls.tmp"
  : > "$TMPFILE"
  
  # Search for XMLs containing GMS whitelist entries
  ...
  # Read found files and patch them
  if [ -s "$TMPFILE" ]; then
    while IFS= read -r XMLFILE; do
      [ -z "$XMLFILE" ] && continue
      [ ! -f "$XMLFILE" ] && continue
      
      # Create module overlay path
      ...
      # Patch - remove GMS whitelist entries
  ...
  # Handle partition layout - move files to proper module structure
  for SUBDIR in product vendor system_ext odm; do
    if [ -d "$MODPATH/$SUBDIR" ]; then
      ui_print "  Moving /$SUBDIR to module overlay..."
  ...
  # Clear GMS cache (helps with delayed notifications)
  ui_print ""
