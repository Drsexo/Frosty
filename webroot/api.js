...
var API = (function () {
  ...
  function uid() {
    return "cb_" + Date.now() + "_" + cbCounter++;
  }
  ...
  async function runJSON(cmd) {
    ...
    try {
      ...
      throw new Error("Bad JSON: " + raw.substring(0, 120));
    }
  }
  ...
  function parseOutput(raw, mode) {
    if (mode === "freeze") {
      var m = raw.match(
        /Disabled:\s*(\d+).*?Re-enabled:\s*(\d+).*?Failed:\s*(\d+)/,
      );
      return {
        status: "ok",
  ...
  async function applyKernelTweaks() {
    return await runJSON(
      "sh " + MODDIR + "/frosty.sh apply_kernel 2>/dev/null",
    );
  }

  async function revertKernelTweaks() {
    return await runJSON(
      "sh " + MODDIR + "/frosty.sh revert_kernel 2>/dev/null",
    );
  }
  ...
  async function toggleSystemProps(enable) {
    return await runJSON(
      "sh " + MODDIR + "/frosty.sh apply_sysprops 2>/dev/null",
    );
  }
  ...
  async function applyRamOptimizer() {
    return await runJSON(
      "sh " + MODDIR + "/frosty.sh ram_optimizer 2>/dev/null",
    );
  }
  ...
  function nativeListPackages(type) {
    try {
      return JSON.parse(ksu.listPackages(type || "user"));
    } catch (e) {
      return [];
    }
  }
  ...
  async function importSettings(filePath) {
    var result = await run(
      "sh " + MODDIR + '/frosty.sh import "' + filePath + '" 2>&1',
    );
  ...
  async function shareBackup(filePath) {
    await run(
      "sh " + MODDIR + '/frosty.sh share_backup "' + filePath + '" 2>&1',
    );
  }
